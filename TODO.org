...
** TODO Real examples in tutorial [0/10]
   ...
   - [ ] audit_error
   - [ ] doublecheck_error
** TODO Move enforce_order up higher in the filter chain
   eg, so the phase and page attributes in participant don't get updated before
   they are given a sequence error.
** TODO Transition from tutorial to experiment [0/5]
   - [ ] Button on tutorial/complete which actually marks participant.tutorial_complete
         and redirects to experiment/wait
   - [ ] Admin page to monitor participants' completion of the tutorial
   - [ ] Admin button to advance phase, enabled when all participants are there
   - [ ] At that point, experiment/wait redirects to experiment/start or whatever
   - [ ] experiment/wait should reload every 5 seconds or so
